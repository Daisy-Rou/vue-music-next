/**
 * 该文件是运行在Node.js端的，获取数据的基本思路是后端代理，即提供接口路由供前端页面使用，
 * 然后在路由内部我们接收到前端的请求之后，在发送Http请求到第三方服务接口，携带相应的请求参数
 * 包括签名的参数字段等等
 * 对于第三方接口返回的数据，我们会做一层数据处理，最终提供给前端的数据，前端可以直接使用，
 * 无需在处理，这样也比较符合真是企业项目的开发规范，即数据的处理放在后端，前端只做数据的渲染和交互
 */
    ...
    get(url, {
      ...
      if (data.code === ERR_OK) {
        ...
        for (let i = 0; i < albumList.length; i++) {
          ...
          const albumItem = {}
          // 推荐歌单数据包括 id username title pic等 字段
